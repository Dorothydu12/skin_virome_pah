...
ggplot(pcoa,aes(x=PCoA1,y=PCoA2,colour=PAM_cluster))+geom_point(size=2.5) + xlab("PCoA1 (37.69%)") + ylab("PCoA1 (14.33%)") +scale_shape_manual(values = c(8,0,1,10,3,4,7,3,11,5,2))+ scale_colour_manual(values=c("#C9716A","#EBBB46","#5FA35D","#6490C3"))+gghighlight::gghighlight(label_key = Sample_type)+facet_wrap(vars(Sample_type))

stat_ellipse(aes(fill=dm_diagnosed),type="norm",geom="polygon",alpha=0.2,color=NA)
##PERMANOVA

adonis2(otu.bry~G$Cutotype_PAM,permu=999)
##anosim
anosim(otu.bry,G$Cutotype_PAM,permutations = 999)  
#rarecurve
